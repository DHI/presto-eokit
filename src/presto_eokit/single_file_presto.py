...
class PrestoFineTuningModel(nn.Module):
    def __init__(self, encoder, head, load_mlp_weights=False, mlp_head_path=None):
        super().__init__()
        self.encoder: Encoder = deepcopy(encoder)
        # Ensure the model is trainable since we may call
        ...
        if load_mlp_weights and mlp_head_path is not None:
            self._load_mlp_weights(mlp_head_path)

    def _load_mlp_weights(self, mlp_head_path):
        """
        Load weights into the MLP head if a pretrained model is available.
        """
        checkpoint = torch.load(mlp_head_path, weights_only=True)
        self.head.load_state_dict(checkpoint["state_dict"])
        print(f"Loaded MLP head weights from {mlp_head_path}")
...
class Presto(nn.Module):
    ...
    def construct_finetuning_model(
        ...
        model = PrestoFineTuningModel(
            self.encoder, head, load_mlp_weights, mlp_head_path
